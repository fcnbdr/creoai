...
export default function ECProPage() {
  const { getToken, logout } = useAuth(true)
  const [jobs, setJobs] = useState<ECProJob[]>([])
  ...
  async function fetchJobs() {
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/ecpro/?${params}`, {
  ...
  async function fetchStats() {
    try {
      const token = getToken()
      const res = await fetch('http://localhost:8000/api/ecpro/stats', {
        headers: { Authorization: token ? `Bearer ${token}` : '' },
      })
      if (res.ok) {
  ...
  async function handleSubmitJob() {
    try {
      const token = getToken()
      const res = await fetch('http://localhost:8000/api/ecpro/', {
        method: 'POST',
        headers: { 
  ...
  async function handleBatchGenerate() {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/api/ecpro/batch-generate', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: token ? `Bearer ${token}` : '' 
  ...
  async function handleDelete(jobId: number) {
    ...
    try {
      const token = getToken()
      const res = await fetch(`http://localhost:8000/api/ecpro/${jobId}`, {
        method: 'DELETE',
        headers: { Authorization: token ? `Bearer ${token}` : '' },
      })
      if (res.ok) {
  ...
  async function handleAudit(jobId: number) {
    try {
      const token = getToken()
      const res = await fetch(`http://localhost:8000/api/ecpro/${jobId}/audit`, {
        method: 'POST',
        headers: { Authorization: token ? `Bearer ${token}` : '' },
      })
      if (res.ok) {
  ...
  function getJobTypeBadge(jobType: string): { variant: 'success' | 'warning' | 'error' | 'info'; text: string; icon: string } {
    switch (jobType) {
      ...
      case 'hashtag': return { variant: 'green', text: '标签生成', icon: '#️⃣' }
      default: return { variant: 'info', text: jobType, icon: '📄' }
    }
  }
      ...
      <div className="container-custom py-8 space-y-6 animate-fade-in">
        ...
        <Card>
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <h1 className="page-title">📢 ECPro内容生成</h1>
              <p className="page-subtitle">AI自动生成营销文案、脚本和标签，支持敏感词检测和合规审核</p>
            </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <option value="copywriting">✍️ 文案生成</option>
              <option value="script">📝 脚本生成</option>
              <option value="hashtag">#️⃣ 标签生成</option>
            </select>
          </div>
        </Card>
              ...
              {jobs.map((job) => {
                ...
                return (
                  <div key={job.id} className="glass-card rounded-3xl p-6 border border-slate-800/50 hover-card transition-all duration-300 group">
                    ...
                    <div className="mb-4">
                      <Badge variant="info">
                        {jobTypeBadge.icon} {jobTypeBadge.text}
                      </Badge>
                    </div>
                    ...
                    {/* Completed: Show Content Preview */}
                    {job.status === 'completed' && job.content_urls && job.content_urls.length > 0 && (
                      <div className="mb-4">
                        <div className="rounded-xl bg-slate-950/50 p-4 border border-slate-800">
                          <p className="text-xs font-medium text-slate-400 mb-2">生成内容：</p>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Badge variant="success">✓ 敏感词检测通过</Badge>
                          <Badge variant="success">✓ 合规检查通过</Badge>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 pt-4 border-t border-slate-800/50">
                      {job.status === 'completed' && (
                        <>
                          <Button
                            variant="secondary"
                            onClick={() => handleAudit(job.id)}
                            className="flex-1 px-3 py-2 text-xs"
                          >
                            🔍 重新审核
                          </Button>
                          <a
                            href={job.content_urls?.[0]}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="btn-secondary px-3 py-2 text-xs block"
                          >
                            👁️ 查看内容
                          </a>
                        </>
